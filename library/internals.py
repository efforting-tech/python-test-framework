...
#NOTE - export was an experiment but it is not needed so it will probaby be removed from here
#TODO - later we may want to store the information in a test representation so that we can print info later instead/also.
def run_test(function, name, local_frame, index=None, skip=False, export=None, should_fail=False, title=None):
	...
	if len(function.__code__.co_consts) >= 2 and title is None:
		...
		class_name, class_doc = function.__code__.co_consts[0], title
	...
	if index is not None:
		class_name = f'{class_name}[{index!r}]'
	...
	if not skip:
		...
		#We could potentially get the docstring here instead but we would have to worry about inherited docstrings
		#print('GET DOC', local_frame.f_locals.get('__doc__'))
